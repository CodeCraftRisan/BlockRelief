...
def generate_large_scale_datasets(num_victims=100, num_volunteers=110):
    ...
    baseline_df.to_csv(os.path.join(data_path, 'National_ID_Database.csv'), index=False)
    volunteer_df.to_csv(os.path.join(data_path, 'volunteer_input.csv'), index=False)
    bank_df.to_csv(os.path.join(data_path, 'bank_accounts.csv'), index=False)

    print(f"Successfully generated 3 files in '{data_path}' folder:")
    print(f"   - National_ID_Database.csv ({len(baseline_df)} rows)")
    print(f"   - volunteer_input.csv ({len(volunteer_df)} rows, {num_fake} fake NIDs)")
    print(f"   - bank_accounts.csv ({len(bank_df)} rows)")
